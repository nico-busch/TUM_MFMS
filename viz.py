...
def viz_zones(df, gdf):
    ...
    vehicle_hrs = (pu / pd.to_timedelta(1, 'h')).rename('vehicle_hrs')
...
def viz_hubs(df, gdf, hubs):
    ...
    pu = df['n_trips'].groupby(['pickup_location']).sum()
    pu_air = df.loc[df['hubs'].notnull(), 'n_trips'].groupby(['pickup_location']).sum()
    pu_ratio = (pu_air / pu).fillna(0)

    gdf = gdf.to_crs({'init': 'epsg:3857'})
    gdf = gdf.join(pu_ratio, how='inner')
    ...
    palette.reverse()
    color_mapper = LinearColorMapper(palette=palette[50:], low=0, high=1)
    tile_provider = get_provider(Vendors.CARTODBPOSITRON_RETINA)
    p = figure(x_axis_location=None, y_axis_location=None)
    p.add_tile(tile_provider)
    zones = p.patches('xs', 'ys', source=source_zones,
                      line_color="white", line_width=0.5,
                      fill_color={'field': 'n_trips', 'transform': color_mapper},
                      alpha=0.75)
    ...
    hover = HoverTool(renderers=[zones],
                      tooltips=[('Borough', '@borough'),
                                ('Zone', '@zone'),
                                ('Pickup Air Trip Ratio', '@n_trips{1.111}')])
    p.add_tools(hover)
    p.add_layout(color_bar, 'below')
    p.hover.point_policy = "follow_mouse"
